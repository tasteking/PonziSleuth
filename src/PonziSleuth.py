...
def record(arg: str, addresses: list, prefix: str):
    ...
    for addr in addresses:
        ...
        try:
            ...
            csv_file = open(csv_path, 'a')
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            writer.writerow({'address': addr, 'decision': e.args[0], 'time': 0, 'token': 0, 'pay': 0})
            csv_file.close()
            continue
        ...
        csv_file = open(csv_path, 'a')
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writerow({'address': addr, 'decision': decision, 'time': span, 'token': prompt_tokens+completion_tokens, 'pay': pay})
        csv_file.close()
        #
        json_file = open(json_path, 'r')
